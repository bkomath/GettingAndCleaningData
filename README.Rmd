...
I used rbind() command to concatenate by row each variable data  into one variable data (activity data, subject data, and feature data). In order words a reduction of variables. See scripts bellow.
   Data for activities 
   ````{r eval=FALSE} actData <- rbind(activityTrainData, activityTestData)```
   Data for subjects
   ```{r eval=FALSE} subjData <- rbind(subjectTrainData, subjectTestData)``` 
   Data for features
   ```{r eval=FALSE} featData <- rbind(featureTrainData, featureTestData)```
...
We need to capture the original names under `features.txt` for "featData".
